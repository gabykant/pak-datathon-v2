    ...
    <article className="rounded-2xl border border-[var(--ink-200)] bg-[var(--sand-50)] p-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h3 className="mt-2 text-xl font-black text-[var(--ink-900)]">
            {problem.type}
          </h3>
      ...
      >
        <div className="grid gap-2 text-sm text-[var(--ink-700)]">
          ...
          <p>
            ...
            {problem.businessImpact.join(' • ')}
          </p>

          <p>
            <span className="font-semibold text-[var(--ink-900)]">
              Volume data indicatif:
            </span>{' '}
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          <DataList
            title="Données navires"
            items={problem.requiredData.vessel}
          />

          <DataList
            title="Données marchandises"
            items={problem.requiredData.cargo}
          />

          <DataList
            title="Données commerce international"
            items={problem.requiredData.trade}
          />

          <DataList
            title="Données camions"
            items={problem.requiredData.truck}
          />
        </div>
        ...
        <div className="mt-4 rounded-xl border border-[var(--ink-200)] bg-white p-4">
          ...
          <p className="mt-2 text-sm text-[var(--ink-700)]">
            {problem.recommendedMethods.join(' • ')}
          </p>
        </div>
      </div>
    </article>
